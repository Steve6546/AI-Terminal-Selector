      ...
      <div className={cn(
        ...
        <div className="flex-shrink-0 mt-1">
          <div className={cn(
            ...
            {isUser ? <User className="w-5 h-5 text-muted-foreground" /> : <Sparkles className="w-5 h-5 text-white" />}
          </div>
        </div>
              ...
              <div className="markdown-content w-full">
                <ReactMarkdown
                  ...
                  components={{
                    code({ node, inline, className, children, ...props }: any) {
                      ...
                      return !inline && match ? (
                        ...
                        <code {...props} className={cn(
                          "bg-white/10 px-1.5 py-0.5 rounded-md font-mono text-[0.85em] text-indigo-300",
                          className
                        )}>
                          {children}
                        </code>
                    ...
                    a: ({href, children}) => <a href={href} className="text-primary hover:text-primary/80 underline underline-offset-4">{children}</a>,
